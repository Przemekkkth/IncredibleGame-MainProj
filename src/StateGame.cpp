...
void StateGame::Draw()
{
    sf::RenderWindow* window = GetStateManager()->GetContext()->m_window;
    if (m_currentLevel->levelType == Level::Type::Valley)
    {
        this->valleyLevelRender(window);
    }
    else if (m_currentLevel->levelType == Level::Type::Cave)
    {
        this->caveLevelRender(window);
    }
    else if (m_currentLevel->levelType == Level::Type::ParkourValley)
    {
        this->parkourValleyLevelRender(window);
    }
    else if (m_currentLevel->levelType == Level::Type::BossLevel)
    {
        this->bossLevelRender(window);
    }

    window->setView(m_playerCamera);

    if (m_isLevelCompleted == true) { window->draw(m_nextLevelDoors); }
    ...
    //     m_afterDeathMenu->render(window);
    // }
}
...
void StateGame::levelDependentUpdate()
{
...
void StateGame::isLevelCompleted()
{
...
void StateGame::caveLevelUpdate()
{
    m_caveLevel->bombsBulletsCollision(m_rifle->getFiredBulletsVector());
}

void StateGame::valleyLevelUpdate()
{
}

void StateGame::parkourLevelUpdate()
{
...
void StateGame::updateAfterDeathMenuButtons(sf::RenderWindow* window)
{
...
void StateGame::deleteAllEnemies()
{
...
void StateGame::updateItems(const float& timerPerFrame)
{
...
void StateGame::updateItemsCollision(Item* item)
{
...
void StateGame::bulletsCollision(Item* item)
{
    this->bulletsTileMapCollision(item);
    this->bulletEnemyCollision(item);
}

void StateGame::bulletsTileMapCollision(Item* item)
{
...
void StateGame::bulletEnemyCollision(Item* item)
{
    int bulletsNumber{ m_rifle->firedBullets() };
...
void StateGame::playerEnemyCollision()
{
    for (int iii{ 0 }; iii < m_enemies.size(); ++iii)
    {
...
void StateGame::updateCreatures(const float& timePerFrame)
{
...
void StateGame::updateCollision(Creature* creature)
{
    this->updateTilesMapCollision(creature);
    creature->updateCollision();
}

void StateGame::updateTilesMapCollision(Creature* creature)
{
...
void StateGame::updateRenderBounds()
{
...
void StateGame::calculateCollisionBounds(sf::Vector2f point, int &fromX, int &toX, int &fromY, int &toY)
{
...
void StateGame::updatePlayerCamera()
{
...
void StateGame::updateEnemySpawn()
{
...
void StateGame::moveBackgroundProportionallyToMap()
{
...
void StateGame::updateMousePositions(sf::View &camera)
{
    //All map position
    ...
    m_mousePositionMap = sf::Mouse::getPosition(*GetStateManager()->GetContext()->m_window);

    m_mousePositionMap.x += viewLeftCorner.x;
    m_mousePositionMap.y += viewLeftCorner.y;

    //Window pos
    m_mousePositionWindow = sf::Mouse::getPosition(*GetStateManager()->GetContext()->m_window);

    //View pos
    m_mousePositionView = GetStateManager()->GetContext()->m_window->mapPixelToCoords(m_mousePositionWindow);
...
void StateGame::initButtons()
{

}

void StateGame::initPauseMenu()
{
    //Pause Menu
...
void StateGame::initBackground()
{
    ...
    moveBackgroundProportionallyToMap();
}
...
void StateGame::renderCreatures(sf::RenderTarget* target, bool addBlendMode)
{
    for (int iii{ 0 } ; iii < m_creatures.size() ; ++iii)
    {
...
void StateGame::renderGUI(sf::RenderTarget* target)
{
    m_player->renderHearts(target);
}

void StateGame::renderItems(sf::RenderTarget* target)
{
    for (auto item : m_items)
    {
        item->render(target);
...
void StateGame::caveLevelRender(sf::RenderTarget* target)
{
    ...
    GetStateManager()->GetContext()->m_window->setView(GetStateManager()->GetContext()->m_window->getDefaultView());
...
void StateGame::valleyLevelRender(sf::RenderTarget* target)
{
...
void StateGame::parkourValleyLevelRender(sf::RenderTarget* target)
{
    target->draw(m_background);
...
void StateGame::bossLevelRender(sf::RenderTarget* target)
{
    target->draw(m_background);
...
void StateGame::checkTileMapBounds(int& fromX, int& toX, int& fromY, int& toY)
{
...
void StateGame::checkPlayerCameraBounds()
{
...
void StateGame::CheckXaxis(float leftCameraBound, float rightCameraBound)
{
...
void StateGame::CheckYaxis(float upCameraBound, float downCameraBound)
{
...
void StateGame::deleteDeadEnemy()
{
...
void StateGame::setLevelAfterBoss()
{
